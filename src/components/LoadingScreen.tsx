...
import { Terminal, Cpu, Database, Globe, Layers, Wifi, Code, Braces } from 'lucide-react';
...
const LoadingScreen = ({ onComplete }: LoadingScreenProps) => {
  ...
  const generateLogs = () => {
    const processes = ['MEM_ALLOC', 'NODE_BOOT', 'SYS_INIT', 'REACT_MOUNT', 'GSAP_TWEEN', 'UI_RENDER', 'DATA_FETCH', 'AUTH_CHECK'];
    // Only generate 4 logs on mobile/tablet so it doesn't clutter, 8 on desktop
    const isSmall = window.innerWidth < 1024;
    const count = isSmall ? 4 : 8;
    return Array.from({ length: count }).map((_, i) => 
      `0x${Math.floor(Math.random() * 16777215).toString(16).padStart(6, '0').toUpperCase()} : ${processes[i]}`
    );
  };
  ...
  useGSAPContext(() => {
    ...
    tl.to('.content-wrapper', {
      ...
      duration: 0.8,
      ease: 'power3.inOut'
    }, "start+=2.6"); // Waits for counter to finish
  ...
  const splitText = (text: string) => {
    return text.split('').map((char, index) => (
      <span key={index} className="main-char inline-block">{char === ' ' ? '\u00A0' : char}</span>
    ));
  };
      ...
      <div className="content-wrapper absolute inset-0 flex flex-col justify-between p-4 sm:p-8 lg:p-16 text-zinc-100 overflow-hidden">
        ...
        <div className="relative z-10 flex justify-between items-end w-full">
          ...
          <div className="text-[20vw] lg:text-[12rem] leading-none font-bold tabular-nums tracking-tighter text-zinc-100 font-sans flex items-baseline">
            ...
            <span className="text-xl md:text-6xl text-zinc-600 ml-1 lg:ml-4 mb-2 lg:mb-8">%</span>
          </div>
        </div>
      </div>
    </div>
